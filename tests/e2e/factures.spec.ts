...
test('post invoice to accounting after emission from list', async ({ page }) => {
  const prisma = createE2EPrisma()
  let associationId: string
  let fiscalYearId: string
  let customerName: string
  ...
  try {
    const assoc = await prisma.association.create({
      data: {
        name: 'Association Factures post E2E',
    ...
    customerName = 'Client E2E post compta'
    await prisma.counterparty.create({
      data: { associationId, kind: 'CUSTOMER', name: customerName },
    })
  } finally {
  ...
  await page.getByText(customerName, { exact: true }).click()
  await page.getByLabel('Nom affiché *').fill(customerName)
  await page.getByLabel('Description ligne 1 *').fill('Stage post compta E2E')
  await page.getByLabel('Montant TTC (€)').fill('80')
  await page.getByLabel('Compte produit *').click()
  await page.getByText('706 - Prestations', { exact: true }).click()
  await page.getByLabel('Envoyer en compta (créance client, non réglée)').uncheck()

  await page.getByRole('button', { name: 'Émettre la facture' }).click()
  await expect(page).toHaveURL(/\/factures$/)

  await expect(page.locator('tbody').getByText('Hors compta', { exact: true })).toBeVisible()

  await page.getByRole('button', { name: 'Envoyer la facture 2026-0001 en compta' }).click()
  await expect(page.locator('tbody').getByText('En compta', { exact: true })).toBeVisible()
})
